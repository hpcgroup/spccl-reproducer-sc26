...
all harnesses here should take <5 minutes. Before building, SparCML
header files should be obtained from https://gitlab.com/rengglic/SparCML/-/tree/b8acd1c2846bf164377c2b94c70828494917061c/micro_benchmark.
These files should be placed in `external/SparCML`.
This includes:

- `c_allreduce_big.h`
- `c_allreduce_recdoubling.h`
- `c_allreduce_ring.h`
- `c_allreduce_small.h`
- `c_common.h`
...
to be changed to an account the user has access to. Each submission produces
one data point per configuration.
...
Backfill data (missing data points used in the ablation not present in
main microbenchmark results or tuning sweeps)
for the ablation figure can be obtained by running the corresponding
16-GPU FP32 experiment under `sbatch_ablation_final/`. Each submission produces
one job-level data point per configuration.
