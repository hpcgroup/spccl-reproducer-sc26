...
Run the launch scripts, which submit the individual experiments with `sbatch`:
...
Each `launch_all.sh` runs all scales across multiple sbatch submissions (8 → 16 → 32 → 64 GPUs).
